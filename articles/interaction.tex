...
The mechanical hardware was co-designed with luthier Roberto Livi and curator of San Colombano Catalina Vicens using the design constraints in Section~\ref{sec:design-constraints}. The hardware was then commissioned for fabrication with Roberto Livi.

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{img/49-key-model-cross-section.pdf}
...
The impact of visual stimulius should not be underplayed, in particular when it comes to musicians.
...
Instead of trying to mitigate this influence, the design embraces it and attempts to leverage it. This phenomenon, akin to a `musical instrument McGurk effect’ \cite{mcgurk_hearing_1976}, is one of the reasons why the electronics have been concealed from view. 

The Mcgurk effect is perhaps an apt example, as the effect itself is proposed to extend beyond speech and into the domain of visual musical cues \cite{saidana_mcgurk_1990}. The
...
\cite{saidana_mcgurk_1990} that Mcgurk not restricted to speech.
\cite{politzer_mcgurk_2019} that
...
\subsubsection{Arduino Nano 33 BLE}\label{nano-ble}

Arduino Nano 33 BLE (Bluetooth Low Energy) microcontroller was chosen for a few reasons. The Nano form factor was
...
The Nano 33 BLE was used in the initial stages for its integrated inertial
measurment unit.
...
fallback. The Nano 33 BLE also has a 12-bit DAC, which provided a contingency
