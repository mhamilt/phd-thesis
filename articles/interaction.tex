...
A major limitation with commercial MIDI interfaces is there reduction of musical expression to simple key attack velocity, of which \textcite{oboe_mikey_2006} write 
...
The dynamics of the attached sound synthesis engine are determined from the velocity at which the key travelled from a rest position (or central trigger point) to being fully pressed. For the piano this is a fair representation as the hammer strike occurs close to the bottom of the keys motion. When this type of interface is applied to the harpsichord, there is a problem of misrepresentation of the instrument. This stems from the fact that most MIDI keyboards trigger there output at the 
end of the key's travel. For the harpsichord, both the sonic and central haptic events happen in the centre of the keys travel. A force acts against the player when the plectrum encounters the string and a second force as the jack bodies push against the jack rail.

Limited or absent haptic feedback is typically framed as a problem for modern musicians using synthesisers. These musicians are being denied physical experience from which they can draw upon and use to inform and shape expression in performance. This absence of haptic information is also true for instruments that cannot be played such as the musical heritage instruments within museum collections.
A major motivation for the design of the interface in this chapter was to overcome this very limitation.

This motivation for the project in part led to the creation of an interface to be designed in a collaboration between Museo San Colombano and the NEMUS project.
...
The interface would be installed of an exhibition at San Colombano to provide interaction with digitised instruments in its collection. The design was for an electronically augmented replica of a historical harpsichord with the goal to enable visitors to experience the
...
Presented in this chapter is the creation of two such digital musical interfaces (DMIs). A core component of the design was retention of haptic information as could be found on XVI century Italian harpsichord. The chapter covers the motivations, constraints and methodology for creating such instruments. The creation of the interfaces is presented in chronological order, beginning with the first prototype., the refinements required before moving onto to full scale iterations of the idea.

The keyboard provides the haptic feedback that would be present on an original 15th century
harpsichord with two 8-foot registers as is typical of the Italian style \cite{tagliavini_clavicembali_1987}. The interface builds on the work of previous musical heritage projects \cite{mcalpine_sampling_2014,baldwin_tromba_2016} as well as haptic keyboard projects from the previous 40 years.
...
% and enhance its likelihood of being perceived by visitors as an equal to the historic instruments within the collection.
...
In addition to enhancing the musical instrument museum visitor experience, the keyboard looks at the process of digital research sustainability. The optical sensing technique for the keyboard is adapted from a similar project on the piano by McPherson \cite{mcpherson_portable_2013}. As such, this project
extends the longevity of the results beyond the scope envisaged in the original
works by building on best practice for digital research archiving \cite{smith_software_2016} as it applies to digital musical instruments (DMIs) and electronics hardware \cite{mcpherson_nimehub_2016,fiordelmondo_longevity_2024}.  The final design iteration aims to serve as an example of a potential research probe to explore the unique characteristics of the harpsichord and its impact on performance. 
...
\subsection{Visitor Engagement and Digital Interaction}
...
Another recent case of introducing interactivity to an heritage instrument collection was the ``Tromba Moderna'' project by \textcite{baldwin_tromba_2016}. The Tromba Moderna was created for an exhibition at the Musikmuseet Musical Instrument Museum in Copenhagen. The exhibition was also to address the problem of engagement and interactivity between visitors and the collection. In this case the issue of musical heritage playability was approached by recreating and augmenting a replica of a historical tromba marina. A piezo transducer was connected to a sound synthesis engine and a driver within the
instrument to simulate the expected vibrations of a historical tromba marina. 
From visitors surveyed, the Tromba Moderna was found to have an overall positive impact on the museum experience.

The motivations for the interfaces in this chapter inherit some aspects from the Tromba Moderna
project. The interface aims to offer a tool to enhance the visitor experience of
the Tagliavini collection while retaining a form of continuity with
historical instrument-building traditions. The projects of \textcite{baldwin_tromba_2016} and \textcite{mcalpine_sampling_2014} are  also a stepping-off point, with the interface aiming to expand upon the ideas of both exhibits.

Methodology and motivation in design for the harpsichord interface has also drawn upon the long history of haptic keyboard research over the previous 40 years.
...
% \textcite{hwang_airpiano_2017} ultrasonic haptics combined with a virtual reality headset and environment. Noted limitation of polyphonic playing.
% \textcite{timmermans_multibody-based_2020} further extending concepts \textcite{oboe_mikey_2006}  noting also the benefit that ``key physical characteristics are preserved'' and notably achieving a haptic device with the use of simple, low-cost electronics.
% One commonality across all of these projects is that they are typically restricted a single or a small number of keys.



% Applying the same here is perhaps in trying to do too much. However, the rise of low-cost electronics at the demonstrated that ``a mass production of an active keyboard is now possible with the proposed approach.''
