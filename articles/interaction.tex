...
49-key, two-register harpsichord keyboard replica. Since the Tagliavini
...
A magnets can be attached to the jack so that it interacts with the reed switch atthe pluck point. 
The reed can also be mounted in such a way to allow for fine adjustment. However, this places calibration in entirely the physical domain and the scalability of such a slution was not viable for the intended function. 
...
This line of enquiry led to following from the work in \textcite{mcpherson_portable_2013, moro_platform_2020}. A system was created using the Fairchild QRE1113 \footnote{Fairchild, now ON Semiconductor,
QRE1113 datasheet
\url{https://www.onsemi.com/download/data-sheet/pdf/qre1113-d.pdf}}. The QRE1113
is an optical sensor composed of an infrared (IR) LED and a phototransistor sensitive to IR light (Figure~\ref{fig:simple-schematic}). The
...
\begin{figure}  
  \centering
  \frame{\includegraphics[width=\linewidth]{img/49-key-model.pdf}}
  \caption{}   
  \label{fig:cross-section}
\end{figure}

To address this restriction, we looked toward another common application of the QRE1113, which is within line following robots \cite{ramesh_turning_2017}. For line following,
...
cutting the tags required, allowing the process process to scale from the six jacks of the prototype to the 98 jacks of Mark 1 model.
...
\begin{figure}
    \centering
    \includegraphics[width=0.75\linewidth]{img/prototype-mini-pcb-bw.png}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{img/3-49-key_scale.pdf}
    \caption{Scale comparison of the 3-key model used for prototyping and the 49-key model for the San Colombano Exhibition.}
    \label{fig:3-49-comparison}
\end{figure}
...
\subsubsection{Visual Perception of Musical Instruments}
...
\textcite{mcalpine_sampling_2014}
...
The number of available analogue-to-digital channels (ADC) on the Arduino is limited to eight. For the prototype, sensors outputs were connected directly to the available ADCs.
One of the first a `problems of scale', introduced when expanding from six jacks of the prototype model to the Mark 1, was how to accommodate 98 optical sensors. To overcome the Arduino's ADC limitation, a CD4051BE Texas Instruments multiplexer was introduced.  A multiplexer (multiplexor, MUX) a circuit that routes a common pin to one of several inputs or output. The MUX would provide a means to connect multiple sensor output signals to a single ADC pin on the Arduino. Available GPIO pins on the Arduino allowed for programmatic control of signal routing. This is the same way in with the ADCs themselves are routed on the Arduino \cite{nordic_nrf52840_2025}, meaning there is a cascade of two mux circuits. An 8-channel multiplexer was used in order to mitigate the problems typically associated with cascading multiplexers, such as increased signal settle time and increased noise. The sensor outputs were then divided between two microcontrollers.  Seven of the available eight channels on the CD4051BE were used, which gavee a clean division of seven sensors per sensor board, 7 sensor boards per register giving coverage to the 49 jacks. In total, 14 sensor boards and two controller boards were installed. Details on the sensor and controller boards are given in the next section.

\subsection{Printed Circuit Boards}

To ensure stable connection, reduce the footprint of the circuit and fix the pitch spacing of the sensors to match the jacks, two printed circuit boards (PCBs) were designed: the sensor board and the controller board.

\subsubsection{Sensor Board}

Sensor board contains components required for the measurment of vertical jack displacement. Dividing the sensors across multiple boards reduced manufacturing costs, allowed for easier installation and provided some room for adjustment in both the distance between sensors and the surface but also the centre point to the corresponding 7 jacks.
...
\begin{figure}[h]{0.4\linewidth}
    \centering
    \includesvg[inkscapelatex=false, width=\linewidth]{img/p}            
    \caption{logarithmic shape}    
    \label{fig:log-harp}
\end{figure}
...
\begin{figure}
    ...
    \caption{Mark 1 Controller board schematic. Power supplied through \texttt{J1}. The controller board provides a reliable means to connect to the sensor board as well as surrounding components such as the non-volatile FRAM.}
    \label{fig:sensor-schematic-1}
\end{figure}
...
%     \centering
%     \includegraphics[height=0.3\linewidth]{img/keys-milled.jpeg} 
%     \caption{Sensor PCB}
%     \label{fig:sensor_v2}
%   \end{subfigure}
%   \begin{subfigure}[h]{1\linewidth}
%     \centering
%     \includegraphics[height=0.1\linewidth]{img/milled-keys.jpg} 
...
\begin{figure}
    ...
    \caption{Plot illustrating hysteresis as two thresholds. Plot shows a full key press and release event. After passing the pluck threshold, the key enters a \texttt{PLUCKED} state. The key then enters the \texttt{RELEASED} state after passing the lower release threshold.}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{img/pcb_sensor_schematic_v2.pdf}
