...
\begin{figure}
    \centering
    \includegraphics[width=0.75\linewidth]{img/pcb-prototype-schematic.png}
...
Figure~\ref{fig:controller-schematic-1} shows the schematic of the controller which illustrates its purpose to facilitate signal routing. Power is supplied through barrel jack \texttt{J1} and there are separate header connectors for the \texttt{RESET} button, \texttt{ROTARY} encoder, and NVM \texttt{FRAM}. The connections to sensor board are separated into \texttt{SIGNALS} for the optical sensor signals and \texttt{SENSOR_BOARD} containing the power, multiplexer channel select and LED control data. On the silkscreen of the final PCB the sensor headers are marked as 

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth,angle=180]{img/pcb-controller-board-v0.1.0.png}
    \caption{Mark 1 controller board.}
    \label{fig:controller-schematic-1}
\end{figure}


The Nano BLE is mounted to the PCB via header sockets. Thisformat, was the central
