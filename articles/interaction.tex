...
displays. This ``red velvet cord'' approach \cite{mcalpine_sampling_2014} protects fragile mechanisms but diminishes the
instruments’ functional identity, disconnecting visitors from the full richness
of their historical and cultural context.

The Tagliavini Collection \cite{meer_collezione_2007},
housed at Museo San Colombano - Genus Bononiae, Bologna, renowned for its
...
after minimal intervention.  However, the delicate mechanisms of these instruments and their historical
...
the curator.  As part of a broader initiative to enhance accessibility in museums, the
...
This initiative led to to a collaboration between Museo San Colombano and the NEMUS project with the shared goal of creating an interface to digital representations of historical instruments. The interface would be installed of an exhibition at San Colombano to provide a faithful recreation of the haptic experience of a XVI century Italian harpsichord. The design was for an electronically augmented replica of a historical harpsichord with the goal to enable visitors to experience the
...
This chapter covers the creation of two such DMIs, beginning first with the motivations, constraints and methodology. The following sections then cover creation of the interfaces in chronological order, beginning with the initial prototypes before moving onto to full scale iterations of the idea. The keyboards provides the haptic feedback that would be present on an original 15th century harpsichord with two 8-foot registers as is typical of the Italian style \cite{tagliavini_clavicembali_1987}. The interface builds on the work of previous musical heritage projects \cite{mcalpine_sampling_2014,baldwin_tromba_2016} as well as haptic keyboard projects from the previous 40 years.
...
state where is is no longer be playable. This is true, even when collections adhere to the
...
them as ``copyable'' as possible. Not all instruments can be copied and it there is then a choice to be made for which instruments are deserving of recreation. 

With digital instruments, the interface is decoupled from the acoustic sound generator and the interface can be one-to-many. Communication with many digital heritage instruments can be achieved from a single interface, which was the approach embraced by \textcite{mcalpine_sampling_2014}. In a case similar to the Tagliavini collection, \textcite{mcalpine_sampling_2014} involved an
