...
Two sets of design principles were drawn up, one for the final mechanical interface and another for the
electronic sensor system. These provided pillars on which to guide the design process and a set of criteria by which the mechanics and electronics could be evaluated.

\subsection{Mechanical Interface}

The initial goal was to create an actuated haptic interface to recreate the tactile and force feedback provide by the harpsichord.

It became clear through the literature that physical feel was only part of the experience. 
The design of the augmented replica keyboard for the Museo San Colombano was guided by three principal constraints stipulated by the
museum:
...
Luckily, with the assistance on Museo San Colombano, a small model containing three harpsichord actions was procured by the NEMUS project (Figure~\ref{fig:3-key-model}). The construction of the model were after a XXXX Italian harpsichord with two registers. The cross-sectional construction permitted easy access to the key lever and jacks directly and therefore allowed simple means of mounting multiple sensors for testing. The multiple keys and registers meant that more complex iterations of the sensor system could be expanded on the same model without having to construct or procure another. The aesthetics of the model also complied with the visual requirememntes as set out in the design priciples.

% Embedding: Once a haptic system had been decided upon and its scalability assessed, the next stage will likely be to embed the haptic system within an existing MIDI controller. This will aim to discover any idiosyncrasies with scaling the problem before fabricating a bespoke housing and enclosure. Keyboards also present a problem of having many parts requiring manufacture. Using an existing unit will help both to cut down on time for fabrication but also cut down on waste.

% Fabrication: The final stage will then aim to create a bespoke enclosure for the final hardware interface. This will likely be a small 2 1/2 octave keyboard both to reduce time to manufacture but also help ensure some level of portability when visiting other institutes.
...
\subsection{QRE1113}\label{qre1113}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{img/all_comparison-90.pdf}
    \caption{Scale drawing comparison of the 1547 Trasuntino, 49-key `Mark 1' model, and the 3-key prototype.}
    \label{fig:compare}
\end{figure}
...
Reference repositories for this project can be found here:
    ...
    \begin{itemize}
        \item 
        \emph{Firmware}:
        \url{https://github.com/Nemus-Project/harpsichord-interface-firmware}
        \item 
        \emph{PCB CAD}:
        \url{https://github.com/Nemus-Project/harpsichord-interface-cad}
        \item 
        \emph{Models}:
        \url{https://github.com/Nemus-Project/harpsichord-interface-models}
    \end{itemize}
