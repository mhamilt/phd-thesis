...
For heritage exhibition the temptation to use new technology as a solution to old problems seems a natural one. 
Creating an actuated system inspired by an interface was a route to take, but with it comes the inherent risk of getting ``too close'' to the original interface \textcite{michon_hybrid_2018}.
The work here builds on what was laid out by \textcite{mcalpine_sampling_2014}.

% As \textcite{michon_hybrid_2018} notes ``by getting too close to the original instrument, we ...[lose] part of its novelty and that it would never be as good as its purely acoustical counterpart.
