...
\begin{quotation}
    To allow others to cite a repository in their work, the Citation File Format (CFF), developed by Druskat [10] and fully supported by GitHub, can be used. Creating a file named CITATION.cff in a project’s root folder and including the necessary metadata makes it easy to cite a repository. The CITATION.cff file can be easily converted to BibTeX or plain text using a dedicated converter [23], which GitHub also has built-in. If a CITATION.cff file is added, a “Cite this repository” command will automatically ap- pear on the right side of the GitHub page. External references (e.g., software, repositories, articles, etc.) can be added to the same file.
\end{quotation}
...
is the most underserved. Any source code can be modified, but to do so one needs to first
understand it and the bare code does not typically provide adequate information.
From the lack of universal version control, reference documentation
we can surmise that there is little intention of external development for these software.
This conclusion is also backed by the absence of DOIs as when the software is not `alive’, there is no reason to cite it directly. Static software can be served by suggested paper citations.


the intentions of the software reflected what is and what is not
present in 


To run is well represented as is the ability to share. The ability to ``study’’
can only go so far as the software itself is documented so to is the ability to
modify. 



We can see  Across the entire set, there
intention that software be used as it is openly available. 

The ``user’’ side of
the user developer spectrum has the greatest focus, as the code is freely
available, and in most cases, the documentation that does exists concerns the
user. The researcher, the musicologist ,the luthier the acousticians, these are
all users of the software, but there seems to be little intention of further
development, at least from eternal sources. This might explain the lack of
version control repository. 

Software that needs to be used just needs the compiled binary, which is why so
often these project are shared as static binaries, the source code serving as
contingency for those systems that are not covered.

Developer documentation is not needed if there is no expectation that project be
developed. It is fair to assume the the code is open so that it can be copied
wholesale into another, not so that it can be, forked, iterated upon and
reintegrated into a project.

From the lack of a VCS we can infer the assumed source of development is
internal, from a department or research project, and not an external
contributor.

The functionality of a pull request removes a burden from an author of software.
Rather than being contact with a suggestion which would have to be implemented
at the cost of time and effort, the pull request allows the external contributor
to identify a problem, suggest a solution and identify exactly where and how
that solution is enacted. There is still oversight required from the repository
owner, but the barrier to entry compared to email address is far lower.

Version control means the potential for development, if no potential is assumed
then it is not needed. Development is not something that happens, but it is
enabled.

The assumed lack Assuming a lack of intern for development is further backed by
the of a DOI. If software is not `alive’ there is not reason to have more than a
suggested citation as it is not going to change. If there is no intention for
software to change, there i no need for per version DOI as it is not going to
changed.

Each example of software constitutes a technical marvel as it is joining of
disparate skillsets to create something that has true multidisciplinary
application. It seems to be a great shame that the assumption. We cannot help
but wonder what opportunities and innovation we are missing out on by the lack
of potential for development.

ITA toolbox and its BSD clause licence, if it cannot be uploaded to a third
party archiving platform. This is where instates could take a more active role
in the archiving process. ITA INSTTITUTUE has a public digital archive, and an
archive and DOI should be able to be generated from there.