...
The proposal was the creation of a tool \textsc{MAGPIE} which would collect together the functionality for computing a discrete biharmonic operator with parametric boundary conditions, an eigensolver, impulse response generation through modal time integration, and Young's modulus estimation.
The first iteration was refactor and functionalise the original 600 line MATLAB script to permit great reusability.

The source was split into five areas of functionality. The first concerned the approximately 90 individual matrix coefficients as they were calculated in isolation. The original source had been designed for a block sparse format (Figure~\ref{fig:biharm-subblocks}). The block format resulted in more manageable source code in the assembly of the discrete biharmonic but at the cost of performance. The intention as to optimise the assembly of the biharmonic, which takes place in the \texttt{bhmat} function. Optimisation would be made easier if the coefficient calculation were separated from the biharmonic assembly stage. Separating this stage into the \texttt{bhmat} function would also ensure optimisation would cascade to any other source code which used it such as the central \texttt{magpie} function. This function aimed to consolidate the 16 parameters of the original script into a standard format of six required arguments representing the plate material density, elastic constant, poisson ratio, dimensions, boundary conditions and grid resolution with a validation stage to ensure the correct type for each argument.
The final functions were for young's modulus estimation and generating an impulse response through modal time integration with loss parameters.
