...
Knowledge of material parameters, particularly elastic constants, provides valuable insight into the structural integrity of a musical instrument. In the case of string instrument soundboards, elastic constants offer a quantitative measure of the instrument’s condition \cite{duran_use_2025}. For collections of musical instruments, understanding the elastic limits of materials is essential for effective conservation \cite[p. 19-20]{barclay_care_1997}. Periodic monitoring of these constants allows curators to assess the state of a soundboard and to intervene when necessary.
...
Plates are particularly well-suited for studying vibration through eigenshape visualization. They are commonly used as educational tools for demonstrating Chladni patterns \cite{chladni_treatise_2015}, providing both accurate results and visually engaging representations. A platform such as \textsc{MAGPIE} can serve dual purposes: as a teaching aid for instructors and as an analysis tool for researchers and industry practitioners. Its functionalities include applying elastic boundary conditions along plate edges, visualizing and exporting eigenshapes and frequencies, performing time-domain simulations of the plate equation for sound synthesis, and estimating the elastic modulus of experimental plates. For conciseness, this work focuses primarily on the eigenvalue problem and its implementation, rather than the full range of capabilities.


% \begin{quotation}
% To describe the vibrational modes of wood plates generally requires four elastic constants. These may be Young's moduli along (Ex) and across (Ey) the grain, the in-plane shear modulus G, and the larger of the two Poisson ratios vxy' Foraquarter-cutplateinthexyplane,Ex= EL,Ey= ER,G= GLR, vxy = VLR , and Vyx = VRL' 
% \end{quotation}
% -- \cite{fletcher_physics_2012}
