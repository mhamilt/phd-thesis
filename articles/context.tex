...
Open software is living in the sense that it capable of constant change, when used in research or research software this can be a problem for cite-ableility and reusability
...
Although Aurora is free digital software, its source code has not historically been open; however it was still in active development by Farina as late as 2023 \cite{bevilacqua_new_2023}.

Without a formal software citation system, Aurora's usage in historical research is difficult to track. Determining the versions of Aurora used can be inferred from the date of a paper but this neither ideal nor accurate. With development to Aurora it is difficult to determine what results would be 

The compiled binaries exist on the Farina's own website with the note:

The repository for the Aurora binaries -- \url{https://www.angelofarina.it/Aurora/download} -- will likely rot without intervention.

As part of this PhD the Audacity variant of Aurora's source code was put onto a public version control repository, under a GPLv3 licence and DOI created \cite{farina_aurora_2025} in line with the guidelines of Software Sustainability institute \cite{hong_how_2019}. Since Audacity was open source, the Aurora software could be embedded directly utilising Audacity's experimental `module' system, which is both under-documented and under-supported \footnote{\url{https://manual.audacityteam.org/man/modules.html}}.

A drawback of this approach is that the Aurora source code has become entangled within that of Audacity's. Breaking changes -- updates to the source code that break the functionality of other code -- are inevitable, with Aurora relying on older versions of Audacity which are compiled `in tandem' to function \footnote{``Modules should be built on the same date as Audacity, otherwise the version string check will prevent the module loading'' \url{https://manual.audacityteam.org/man/modules.html}}.

A version of Aurora entangled with the Audacity software \cite{farina_aurora_2025} which provides a code base from which a working library could be made.

Lessons learned from this and best practice suggest to take a separate library / interface approach which has guided the structure of MAGPIE (Chapter~\ref{chapter:magpie}.

Aurora is not alone, by way of example, look at the YIN source code from the paper \cite{cheveigne_yin-a-fundamental_2002} 
...
Software can be free like Aurora or free and open source like YIN, but this does not make invulnerable to rot.

Just one example of link rot but for an algorithm that is widely cited. DOIs and independent digital repositories would go some way to address this.

Aurora and YIN represents only two data points of research software within the field of acoustics. 
